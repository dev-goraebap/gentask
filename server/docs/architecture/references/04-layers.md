...
프레젠테이션과 애플리케이션은 같은 `application` 패키지에 있습니다. 계층이 다른데 패키지가 같은 이유는 컨트롤러와 그것이 호출하는 서비스가 항상 함께 변경되기 때문이며, 둘을 가르는 것은 폴더가 아니라 클래스의 역할입니다. 식별 규약은 [03. 코드 작성 규약](03-code-style.md)이 소유합니다.
