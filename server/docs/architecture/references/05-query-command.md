...
#### 우회 팩토리로 피할 수 있지만 대가가 있습니다

값 객체에 검증을 건너뛰는 생성 경로를 하나 더 두고 조회가 그것을 쓰면 이 문제만은 사라집니다.

```java
static TaskTitle trusted(String value) { ... }   // 검증하지 않는다
```

그 순간 **`Task` 라는 타입이 보장하는 것이 깎입니다.** 지금은 명제가 하나입니다 — "인스턴스가 존재한다 = 그 값이 한때 불변식을 통과했다". `restore` 가 예외인 것은 *저장된 것은 한때 통과했다* 로 정당화되고, **호출자가 저장소 하나**라는 규약이 그것을 지킵니다.

우회 경로는 값 객체마다 하나씩 늘어납니다. 그것이 공개되면 명령 경로에서도 쓸 수 있게 되고, 그때 불변식은 **컴파일도 테스트도 통과한 채로** 빠집니다. 검증을 건너뛰는 생성자가 늘수록 그 타입은 "무엇이든 담을 수 있는 것" 에 가까워집니다.

#### 검증 재실행은 근거 하나일 뿐입니다

우회 팩토리로 그것을 피해도 나머지는 남습니다.

| 근거 | 우회 팩토리로 해결되는가 |
| :--- | :--- |
| 저장된 값이 검증을 다시 지납니다 | 해결됩니다 |
| 도메인 타입이 응답의 JSON 형태를 정합니다 | 아닙니다 |
| 화면 요구가 애그리거트에 필드를 붙입니다 | 아닙니다 |
| 조회가 애그리거트 경계에 묶입니다 | 아닙니다 |

**마지막이 가장 큽니다.** 애그리거트는 트랜잭션 경계이고 조회는 그 경계를 넘나들어야 합니다. 목록에 다른 모듈의 표시명이 필요해지면 애그리거트로 받는 구성에서는 양쪽을 각각 로드해 코드에서 붙이게 되는데, 그것이 5절이 금지하는 형태입니다.
