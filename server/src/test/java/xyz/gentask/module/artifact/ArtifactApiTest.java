...
class ArtifactApiTest {
    ...
    @Test
    @DisplayName("아티팩트 생성 시 아티팩트와 1차 개정 레코드를 함께 등록한다")
    void 세우면_첫_개정이_함께_선다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"정한 것\",\"body\":\"본문\"}");
    ...
    void 본문_없이_세우면_빈_본문이다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"제목만\"}");
    ...
    @Test
    @DisplayName("제목이 공백이면 400 오류를 반환하고 아티팩트를 생성하지 않는다")
    void 제목이_비어_있으면_담지_않는다() throws Exception {
        mockMvc.perform(post("/api/v1/projects/{projectId}/artifacts", projectId)
                        .cookie(session)
    ...
    @Test
    @DisplayName("동일한 제목의 아티팩트가 존재해도 중복 생성을 허용한다")
    void 같은_제목을_막지_않는다() throws Exception {
        아티팩트를_세운다("{\"title\":\"겹치는 제목\"}");
        아티팩트를_세운다("{\"title\":\"겹치는 제목\"}");
    ...
    @Test
    @DisplayName("폴더 미지정 시 최상위 루트 경로에 아티팩트를 생성한다")
    void 폴더_없이_세우면_뿌리에_선다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"뿌리의 것\"}");

        상세(artifactId).andExpect(jsonPath("$.summary.folderId").value(nullValue()));
        목록().andExpect(jsonPath("$[0].folderId").value(nullValue()));
    }

    @Test
    @DisplayName("폴더를 지정하여 생성하면 해당 폴더 하위에 아티팩트를 등록한다")
    void 폴더를_적어_세운다() throws Exception {
        String folderId = 폴더를_세운다("담을 자리");

        String artifactId = 아티팩트를_세운다("{\"title\":\"담긴 것\",\"folderId\":\"%s\"}".formatted(folderId));

        상세(artifactId).andExpect(jsonPath("$.summary.folderId").value(folderId));
    ...
    @Test
    @DisplayName("아티팩트의 폴더를 변경하면 소속 폴더 정보가 갱신된다")
    void 아티팩트를_폴더로_옮긴다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"옮길 것\"}");
        String folderId = 폴더를_세운다("받을 자리");

        옮긴다(artifactId, "{\"folderId\":\"%s\"}".formatted(folderId));

        상세(artifactId).andExpect(jsonPath("$.summary.folderId").value(folderId));
    }

    /*
     * 개정이 갖는 것은 아티팩트가 말하는 바이고 어느 폴더에 있는지는 그것과 무관하다(DOC-006).
     */
    @Test
    @DisplayName("아티팩트 이동 시 개정 이력을 추가하지 않고 내용도 유지한다")
    void 옮기는_것은_개정이_아니다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"옮길 것\",\"body\":\"본문\"}");
        String folderId = 폴더를_세운다("받을 자리");
    ...
    @Test
    @DisplayName("아티팩트 폴더를 null로 지정하면 최상위 루트로 이동한다")
    void 아티팩트를_최상위로_옮긴다() throws Exception {
        String folderId = 폴더를_세운다("담을 자리");
        String artifactId = 아티팩트를_세운다("{\"title\":\"옮길 것\",\"folderId\":\"%s\"}".formatted(folderId));

        옮긴다(artifactId, "{\"folderId\":null}");
    ...
    void 있던_자리로_옮기면_그대로다() throws Exception {
        String folderId = 폴더를_세운다("담을 자리");
        String artifactId = 아티팩트를_세운다("{\"title\":\"그대로\",\"folderId\":\"%s\"}".formatted(folderId));
    ...
    void 없는_자리로는_옮기지_못한다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"그대로\"}");

        mockMvc.perform(put("/api/v1/projects/{projectId}/artifacts/{artifactId}/folder", projectId, artifactId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("타 프로젝트 소속 폴더로는 아티팩트를 이동할 수 없다")
    void 남의_폴더로는_옮기지_못한다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"내 것\"}");
    ...
    @Test
    @DisplayName("타 사용자의 프로젝트 아티팩트 이동 요청 시 404를 반환한다")
    void 남의_아티팩트는_옮기지_못한다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"내 것\"}");
    ...
    @Test
    @DisplayName("아티팩트 목록 조회 시 현재 프로젝트의 아티팩트만 반환한다")
    void 목록은_지금_프로젝트의_것만_낸다() throws Exception {
        아티팩트를_세운다("{\"title\":\"이 프로젝트의 것\"}");

        String other = 프로젝트를_세운다(session, "Other", "OT");

        목록().andExpect(jsonPath("$", hasSize(1)));
        mockMvc.perform(get("/api/v1/projects/{projectId}/artifacts", other).cookie(session))
                .andExpect(jsonPath("$", hasSize(0)));
    }

    @Test
    @DisplayName("타 사용자의 프로젝트에 아티팩트 생성 요청 시 404를 반환한다")
    ...
    @Test
    @DisplayName("아티팩트 상세 조회 시 현재 유효한 최신 개정의 본문을 반환한다")
    void 상세는_지금_참인_개정을_낸다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"고칠 것\",\"body\":\"처음 본문\"}");

        고친다(artifactId, "{\"title\":\"고칠 것\",\"body\":\"고친 본문\",\"comment\":null}");
    ...
    void 본문을_원문_그대로_낸다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"원문\",\"body\":\"<script>alert(1)</script>\"}");

        상세(artifactId).andExpect(jsonPath("$.body").value("<script>alert(1)</script>"));
    }

    @Test
    @DisplayName("사용자의 프로젝트에 속하지 않으면 없는 것으로 낸다")
    void 남의_아티팩트는_열지_못한다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"내 것\"}");

        Cookie other = AuthTestSupport.가입한다(mockMvc, mail, "other-" + UUID.randomUUID() + "@example.com");

        mockMvc.perform(get("/api/v1/projects/{projectId}/artifacts/{artifactId}", projectId, artifactId)
                        .cookie(other))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("PROJECT_NOT_FOUND"));
    }
    ...
    @Test
    @DisplayName("타 프로젝트 경로에서 아티팩트 조회 시 404를 반환한다")
    void 다른_프로젝트에서는_없는_것으로_낸다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"이 프로젝트의 것\"}");

        String other = 프로젝트를_세운다(session, "Other", "OT");

        mockMvc.perform(get("/api/v1/projects/{projectId}/artifacts/{artifactId}", other, artifactId)
                        .cookie(session))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("아티팩트 수정 시 신규 개정을 등록하고 최신 개정 번호를 갱신한다")
    void 고치면_새_개정을_가리킨다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"처음 제목\",\"body\":\"처음 본문\"}");

        고친다(artifactId, "{\"title\":\"고친 제목\",\"body\":\"고친 본문\",\"comment\":\"왜 고쳤는지\"}");
    ...
    void 제목만_고쳐도_개정이_된다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"처음 제목\",\"body\":\"그대로 둘 본문\"}");
    ...
    void 앞의_개정을_그대로_둔다() throws Exception {
        ...
        고친다(artifactId, "{\"title\":\"고친 제목\",\"body\":\"고친 본문\",\"comment\":null}");

        List<ArtifactRevisionsRecord> revisions = 개정들(artifactId);
    ...
    void 달라지지_않은_저장은_개정을_만들지_않는다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"그대로\",\"body\":\"그대로인 본문\"}");
    ...
    void 제목이_비면_개정을_남기지_않는다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"그대로 둘 것\",\"body\":\"본문\"}");

        mockMvc.perform(patch("/api/v1/projects/{projectId}/artifacts/{artifactId}", projectId, artifactId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void 사유_없이_담는다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"사유 없이\",\"body\":\"처음\"}");
    ...
    @Test
    @DisplayName("타 사용자의 프로젝트 아티팩트 수정 요청 시 404를 반환한다")
    void 남의_아티팩트는_고치지_못한다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"내 것\",\"body\":\"내 본문\"}");
        ...
        상세(artifactId).andExpect(jsonPath("$.body").value("내 본문"));
    }

    @Test
    @DisplayName("유효하지 않은 형식의 아티팩트 식별자로 조회 시 404를 반환한다")
    void 모양이_아닌_식별자는_없는_것으로_낸다() throws Exception {
        mockMvc.perform(get("/api/v1/projects/{projectId}/artifacts/{artifactId}", projectId, "아티팩트-아님")
                        .cookie(session))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("ARTIFACT_NOT_FOUND"));
    }
    ...
    @Test
    @DisplayName("인증되지 않은 사용자의 아티팩트 접근을 거부한다")
    void 로그인_없이_아티팩트에_닿을_수_없다() throws Exception {
        mockMvc.perform(get("/api/v1/projects/{projectId}/artifacts", projectId))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.code").value("UNAUTHENTICATED"));
    }

    @Test
    @DisplayName("아티팩트 개정 이력 조회 시 최신 개정 순으로 버전 번호, 일시, 작성자, 사유를 반환한다")
    ...
    void 이력은_본문을_싣지_않는다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"제목\",\"body\":\"긴 본문\"}");

        이력(artifactId).andExpect(jsonPath("$.items[0].body").doesNotExist());
    }

    @Test
    @DisplayName("수정 이력이 없는 아티팩트는 초기 개정 1건만 반환한다")
    void 고치지_않았으면_개정이_하나다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"그대로\",\"body\":\"본문\"}");
    ...
    void 이력을_쪽으로_나눠_낸다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"제목\",\"body\":\"본문 1\"}");
        고친다(artifactId, "{\"title\":\"제목\",\"body\":\"본문 2\"}");
        고친다(artifactId, "{\"title\":\"제목\",\"body\":\"본문 3\"}");

        이력(artifactId, "?page=0&size=2")
    ...
    @Test
    @DisplayName("타 사용자의 프로젝트 아티팩트 이력 조회 시 404를 반환한다")
    void 남의_아티팩트의_이력에는_닿을_수_없다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"내 것\",\"body\":\"내 본문\"}");
    ...
    void 다른_프로젝트에서는_이력이_없다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"이 프로젝트의 것\",\"body\":\"본문\"}");
    ...
    void 되돌린_이유를_적을_수_있다() throws Exception {
        ...
        되돌린다(artifactId, 1, "{\"comment\":\"고친 쪽이 틀렸다\"}");

        이력(artifactId).andExpect(jsonPath("$.items[0].comment").value("고친 쪽이 틀렸다"));
    }
    ...
    @Test
    @DisplayName("되돌린 아티팩트를 재차 이전 개정으로 롤백할 수 있으며 신규 개정으로 추가된다")
    void 되돌린_것을_다시_되돌린다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"처음 제목\",\"body\":\"처음 본문\"}");
        고친다(artifactId, "{\"title\":\"고친 제목\",\"body\":\"고친 본문\",\"comment\":null}");

        되돌린다(artifactId, 1, null);
        되돌린다(artifactId, 2, null);
    ...
    @Test
    @DisplayName("타 사용자의 프로젝트 아티팩트 롤백 요청 시 404를 반환한다")
    void 남의_아티팩트는_되돌리지_못한다() throws Exception {
        String artifactId = 아티팩트를_세운다("{\"title\":\"내 것\",\"body\":\"내 본문\"}");
        고친다(artifactId, "{\"title\":\"내 것\",\"body\":\"고친 본문\",\"comment\":null}");
        ...
        상세(artifactId).andExpect(jsonPath("$.body").value("고친 본문"));
    }
    ...
    // --- 준비 --------------------------------------------------------------------------------------------------------
    private ResultActions 이력(String artifactId) throws Exception {
        return 이력(artifactId, "");
    }
    ...
    private void 옮긴다(String artifactId, String body) throws Exception {
        mockMvc.perform(put("/api/v1/projects/{projectId}/artifacts/{artifactId}/folder", projectId, artifactId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
    ...
    private String 폴더를_세운다(String inProjectId, String name) throws Exception {
        String location =
                requireNonNull(mockMvc.perform(post("/api/v1/projects/{projectId}/artifact-folders", inProjectId)
                                .cookie(session)
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    private ResultActions 목록() throws Exception {
        return mockMvc.perform(
                        get("/api/v1/projects/{projectId}/artifacts", projectId).cookie(session))
                .andExpect(status().isOk());
    }

    private ResultActions 상세(String artifactId) throws Exception {
        return mockMvc.perform(get("/api/v1/projects/{projectId}/artifacts/{artifactId}", projectId, artifactId)
                        .cookie(session))
                .andExpect(status().isOk());
    }

    private void 고친다(String artifactId, String body) throws Exception {
        mockMvc.perform(patch("/api/v1/projects/{projectId}/artifacts/{artifactId}", projectId, artifactId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
    ...
    private String 아티팩트를_세운다(String body) throws Exception {
        String location = requireNonNull(mockMvc.perform(post("/api/v1/projects/{projectId}/artifacts", projectId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isCreated())
