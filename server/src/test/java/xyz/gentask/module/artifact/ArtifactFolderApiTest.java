...
class ArtifactFolderApiTest {
    ...
    void 이름이_비어_있으면_세우지_않는다() throws Exception {
        mockMvc.perform(post("/api/v1/projects/{projectId}/artifact-folders", projectId)
                        .cookie(session)
    ...
    void 빈_이름으로는_바꾸지_못한다() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/projects/{projectId}/artifact-folders/{folderId}", projectId, folderId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"   \"}"))
                .andExpect(status().isBadRequest());
    ...
    @Test
    @DisplayName("폴더를 이동하면 소속 하위 아티팩트 및 자식 폴더가 함께 이동한다")
    void 옮기면_담긴_것이_함께_간다() throws Exception {
        String moving = 폴더를_세운다("{\"name\":\"옮길 것\"}");
        폴더를_세운다("{\"name\":\"하위\",\"parentId\":\"%s\"}".formatted(moving));
        String artifactId = 아티팩트를_세운다("{\"title\":\"담긴 아티팩트\",\"folderId\":\"%s\"}".formatted(moving));
        ...
        아티팩트상세(artifactId).andExpect(jsonPath("$.summary.folderId").value(moving));
    }
    ...
    void 자기_자신_아래로는_옮기지_못한다() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/projects/{projectId}/artifact-folders/{folderId}/parent", projectId, folderId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void 자손_아래로는_옮기지_못한다() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/projects/{projectId}/artifact-folders/{folderId}/parent", projectId, grandparent)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("폴더 삭제 시 소속 아티팩트와 하위 폴더는 상위 계층으로 승격된다")
    void 지우면_담긴_것이_한_단계_위로_올라간다() throws Exception {
        ...
        String artifactId = 아티팩트를_세운다("{\"title\":\"담긴 아티팩트\",\"folderId\":\"%s\"}".formatted(folderId));

        지운다(folderId);

        폴더목록().andExpect(jsonPath("$", hasSize(2)))
                .andExpect(jsonPath("$[?(@.name == '하위')].parentId").value(grandparent));
        아티팩트상세(artifactId).andExpect(jsonPath("$.summary.folderId").value(grandparent));
    }
    ...
    void 최상위를_지우면_뿌리로_올라간다() throws Exception {
        ...
        폴더목록().andExpect(jsonPath("$", hasSize(0)));
        아티팩트상세(artifactId).andExpect(jsonPath("$.summary.folderId").value(nullValue()));
    }
     ...
     */
    @Test
    @DisplayName("폴더 목록 조회 시 직속 하위 아티팩트 수와 폴더 수를 포함한다")
    void 담긴_것의_수를_센다() throws Exception {
        String folderId = 폴더를_세운다("{\"name\":\"담은 것이 있는 자리\"}");
        폴더를_세운다("{\"name\":\"하위\",\"parentId\":\"%s\"}".formatted(folderId));
        아티팩트를_세운다("{\"title\":\"하나\",\"folderId\":\"%s\"}".formatted(folderId));
        아티팩트를_세운다("{\"title\":\"둘\",\"folderId\":\"%s\"}".formatted(folderId));

        폴더목록().andExpect(jsonPath("$[?(@.name == '담은 것이 있는 자리')].artifactCount").value(2))
                .andExpect(jsonPath("$[?(@.name == '담은 것이 있는 자리')].folderCount").value(1))
                .andExpect(jsonPath("$[?(@.name == '하위')].artifactCount").value(0));
    }
    ...
    void 목록은_지금_프로젝트의_것만_낸다() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/projects/{projectId}/artifact-folders", other)
                        .cookie(session))
    ...
    void 남의_폴더에는_닿지_못한다() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/projects/{projectId}/artifact-folders/{folderId}", other, folderId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"바꾸기\"}"))
    ...
    void 이미_지워진_폴더는_없는_것으로_낸다() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/projects/{projectId}/artifact-folders/{folderId}", projectId, folderId)
                        .cookie(session))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("FOLDER_NOT_FOUND"));
    }
    ...
    void 모양이_아닌_식별자는_없는_것으로_낸다() throws Exception {
        mockMvc.perform(delete("/api/v1/projects/{projectId}/artifact-folders/{folderId}", projectId, "폴더-아님")
                        .cookie(session))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("FOLDER_NOT_FOUND"));
    }
    ...
    void 로그인_없이_폴더에_닿을_수_없다() throws Exception {
        mockMvc.perform(get("/api/v1/projects/{projectId}/artifact-folders", projectId))
    ...
    private ResultActions 폴더목록() throws Exception {
        return mockMvc.perform(get("/api/v1/projects/{projectId}/artifact-folders", projectId)
                        .cookie(session))
    ...
    private String 폴더를_세운다(String body) throws Exception {
        String location =
                requireNonNull(mockMvc.perform(post("/api/v1/projects/{projectId}/artifact-folders", projectId)
                                .cookie(session)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(body))
