package xyz.gentask.module.issue;
...
class IssueApiTest {
    ...
    @Test
    @DisplayName("프로젝트 수정 후에도 이슈 번호와 개수 집계를 유지한다")
    void 프로젝트_수정은_이슈_번호를_초기화하지_않는다() throws Exception {
        작업_아이템을_세운다("{\"title\":\"첫 항목\"}");
        mockMvc.perform(patch("/api/v1/projects/{projectId}", projectId)
                        .cookie(session)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"변경한 프로젝트\"}"))
                .andExpect(status().isNoContent());
        int number = 작업_아이템을_세운다("{\"title\":\"두 번째 항목\"}");
        상세(number).andExpect(jsonPath("$.summary.number").value(2));
