...
/**
 * 첨부가 놓이는 자리. (owner_type, name) 쌍 하나가 자리 하나이며 그 자리의 정책을 함께 갖는다.
 *
 * 공개 언어(Published Language)다. 첨부를 붙이는 모듈은 이 열거의 값 하나를 고르는 것으로 어디에
 * 무엇을 붙일지 지목한다.
 *
 * 개수와 크기를 표 제약으로 적을 수 없어 여기에 둔다. 하나만 허용하는 자리와 여럿을 허용하는 자리가
...
public enum AttachmentSlot {
    ...
    /** attachments.owner_type 에 저장되는 값. 붙은 레코드의 종류다. */
    public String ownerType() {
        return ownerType;
    }

    /** attachments.name 에 저장되는 값. 같은 레코드에 붙는 첨부의 용도를 가른다. */
    public String attachmentName() {
        return name;
    }

    /** 보관소 키의 접두어 — <prefix>/<yyyy>/<MM>/<blobId>. */
