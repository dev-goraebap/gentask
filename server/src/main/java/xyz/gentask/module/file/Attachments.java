 ...
 *
 * 자리 발급은 여기 없다. 그것은 어느 레코드에 붙을지와 무관한 일이라 file 의 컨트롤러가 직접 받으며,
 * 이 인터페이스는 발급된 것을 도메인 레코드에 매는 일부터 다룬다.
 *
 * 그 레코드에 손댈 자격이 있는지는 판정하지 않는다. 부르는 쪽이 자기 도메인 규칙으로 이미 판정한 뒤
 * 부르며, file 은 그 판정을 되풀이할 근거를 갖지 않는다. actorId 를 받는 것은 자격을 다시 보기
