...
public interface ArtifactRepository {
     ...
     */
    Optional<Artifact> findById(UUID projectId, UUID artifactId);

    /**
     * 특정 폴더에 소속된 모든 아티팩트를 조회한다. 폴더 삭제 시 상위 승격을 위해 논리 삭제된 아티팩트도 포함한다(DOC-008 A7).
     */
    List<Artifact> findAllInFolder(UUID folderId);
    ...
    Optional<ArtifactRevision> findRevisionByNo(UUID artifactId, int revisionNo);
}
