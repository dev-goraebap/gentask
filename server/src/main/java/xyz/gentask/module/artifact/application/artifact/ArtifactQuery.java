...
public interface ArtifactQuery {
     ...
     */
    List<FolderSummary> findFolders(ArtifactScope projectId);

    Optional<ArtifactView> findOne(ArtifactScope projectId, String artifactId);
     ...
     */
    List<VersionSummary> findRevisions(ArtifactScope projectId, String artifactId, int limit, int offset);

    /** 이 아티팩트의 개정 수. 지워졌거나 남의 것이면 0 이다. */
    long countRevisions(ArtifactScope projectId, String artifactId);

    /** 개정 하나. 그때의 제목과 본문을 낸다(DOC-004). */
    Optional<VersionView> findRevision(ArtifactScope projectId, String artifactId, int revisionNo);
}
