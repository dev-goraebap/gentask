...
public interface ArtifactQuery {
    ...
    List<ArtifactSummary> findAll(UUID projectId);
    ...
    Optional<ArtifactView> findOne(UUID projectId, UUID artifactId);
     ...
     */
    List<RevisionSummary> findRevisions(UUID projectId, UUID artifactId, int limit, int offset);

    /** 이 아티팩트의 개정 수. 지워졌거나 남의 것이면 0 이다. */
    long countRevisions(UUID projectId, UUID artifactId);

    /** 개정 하나. 그때의 제목과 본문을 낸다(DOC-004). */
    Optional<RevisionView> findRevision(UUID projectId, UUID artifactId, int revisionNo);
}
