...
import dev.goraebap.devkit.auth.application.registration.OtpIssueRateLimiter;
...
/**
 * 고정 윈도우 인메모리 rate limiter (결정-0015 §결정 3).
 *
 * <p>단일 인스턴스 전제의 키트 구현이다 — 여러 인스턴스로 배포하는 파생 프로젝트는 공유 저장소
 * (Redis 등) 구현으로 교체한다. 포트 뒤에 있으므로 교체는 이 클래스 하나다.
 */
@Component
class InMemoryOtpRateLimiter implements OtpIssueRateLimiter {
    ...
    InMemoryOtpRateLimiter(AuthProperties properties, Clock clock) {
        this.properties = properties;
        this.clock = clock;
    }

    @Override
    public boolean tryAcquire(String ipAddress, String emailNormalized) {
        AuthProperties.Otp otp = properties.otp();
        Instant now = clock.instant();
        cleanupIfBloated(now);
        boolean ipAllowed = acquire("ip:" + ipAddress, otp.ipLimit(), otp.ipWindow(), now);
        boolean emailAllowed = acquire("email:" + emailNormalized, otp.emailLimit(), otp.emailWindow(), now);
        return ipAllowed && emailAllowed;
    }

    private boolean acquire(String key, int limit, Duration window, Instant now) {
        Window current = counters.compute(key, (k, existing) -> {
