...
요구사항은 [유스케이스 모델](docs/1.analysis/유스케이스-모델.md)에 유스케이스 넷(TK-001~004)으로 등록되어 있고 서술서는 대체 흐름까지 구체화된 상태입니다. 화면은 추가 · 보기 · 편집 · 완료의 기본 경로를 목 데이터 위에서 이미 구현하고 있으며, 적는 자리는 상자 안의 아이콘 줄에서 기한과 미리 알림을 붙입니다. 상세는 셸의 오른쪽 `aside` 슬롯에 실리고 열린 항목을 쿼리 파라미터가 갖습니다. 백로그는 `backlog/` 에 발견 Epic 1 · Story 13 · 스파이크 Task 1 로 있고, GitHub 이슈는 쓰지 않습니다. 스파이크(화면 프로토타입)의 남은 질문은 비어 있으며, Story 는 백엔드 연결 전이라 하나도 닫히지 않았습니다. 미리 알림은 붙이는 데까지(ST-013) 화면에 있고 정한 시각에 울리는 쪽은 나중 구획에 있습니다. 반복은 한 줄로만 있습니다. 기획은 모델의 배경 · 범위 밖 절이 대신하며 `docs/2.design/` 은 비어 있습니다.
