...
원리만 보면 포인터 특성이 더 정확한 축입니다. 바텀시트는 화면이 좁아서가 아니라 손가락으로 조작하고 호버가 없어서 필요합니다. 그럼에도 채택하지 않습니다 — **브라우저가 이 값을 신뢰할 수 있게 보고하지 않습니다.**
...
네이티브 위임(터치에서 `<input type="date">` 등 OS 기본 UI 사용)은 **채택하지 않습니다.** OS 와 브라우저마다 외형이 달라져 화면 통제를 잃습니다. 그 결과 날짜 선택기 같은 컴포넌트를 직접 만들어야 하며, 접근성 부담은 brain 이 제공하는 프리미티브로 흡수합니다.

같은 근거가 [03. 코드 작성 규약](03-code-style.md)의 폰트 관련 판단과 이어집니다. 화면이 기기마다 다르게 보이는 것을 허용하지 않는다는 원칙입니다.
...
| **Dialog (입력을 담는 것)** | 중앙 모달 | 풀스크린 또는 바텀시트 | B | 좌우 여백이 남지 않으며 가상 키보드가 본문을 가립니다 |
...
| **확인 대화 (Alert dialog)** | 폭과 무관하게 중앙 모달로 띄웁니다. 5절의 Dialog 행은 입력을 담는 대화에만 적용합니다 |
...
확인 대화를 뺀 근거는 5절이 Dialog 에 패턴 B 를 요구한 **사유가 성립하지 않는다**는 것입니다. 그 사유는 좌우 여백 부족과 가상 키보드의 본문 가림 둘인데, 확인 대화는 문장 한 줄과 버튼 둘만 담아 입력란을 갖지 않으므로 키보드가 뜨지 않고, 좁은 화면에서도 상자가 뷰포트 폭에 닿지 않습니다. 390px 뷰포트에서 대화 폭 212px 로 실측했습니다. 사유가 없는 자리에 표현을 두 벌 만들면 유지 대상만 늘어납니다.
