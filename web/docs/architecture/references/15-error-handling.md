...
401 만 인터셉터가 이동까지 처리합니다. 세션이 끊긴 상태에서 화면이 각자 판단하면 여러 화면이 동시에 로그인으로 이동시키려 합니다.
...
400·403 과 같은 클라이언트 입력 오류나 정상적인 인가 거부는 관측 도구로 전송하지 않습니다. 불필요한 로그가 누적되어 실제 시스템 결함 식별을 방해합니다.
