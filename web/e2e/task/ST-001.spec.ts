...
test.describe('ST-001 제목만으로 작업 추가', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/tasks/all');
    await expect(page.getByPlaceholder('작업 추가')).toBeVisible();
  });

  test('ST-001 AC1: 제목을 적고 확정하면 목록에 그 작업이 있다', async ({ page }) => {
  ...
  test('ST-001 AC2: 제목이 공백뿐이면 작업을 넣지 않는다', async ({ page }) => {
    const 입력 = page.getByPlaceholder('작업 추가');
  ...
  test('ST-001 AC3: 추가를 그만두면 목록을 그대로 둔다', async ({ page }) => {
    const 입력 = page.getByPlaceholder('작업 추가');
    const 이전 = await page.getByRole('listitem').count();
  ...
  for (const [스마트목록, 경로] of [
    ...
    test(`ST-001 AC4: ${스마트목록} 목록을 보는 중에 적으면 그 성질이 붙는다`, async ({ page }) => {
      await page.goto(경로);
  ...
  test('ST-001 AC5: 제목이 공백뿐이면 오류를 알리지 않는다', async ({ page }) => {
    const 입력 = page.getByPlaceholder('작업 추가');
