...
test.describe('ST-018 작업에 파일 붙이기', () => {
  test('ST-018 AC1: 파일을 붙이면 목록에 이름과 크기가 온다', async ({ page, request }) => {
  ...
  test('ST-018 AC3: 붙인 파일을 받으면 올린 그것이 그 이름으로 온다', async ({ page, request }) => {
  ...
  test('ST-018 AC4: 파일을 떼면 목록과 보관소에서 함께 사라진다', async ({ page, request }) => {
  ...
  for (const { 사유, 파일들, 문구 } of 거절될_붙임) {
    test(`ST-018 AC2: ${사유} 그 붙임이 목록에 담기지 않는다`, async ({ page, request }) => {
    ...
    test(`ST-018 AC5: ${사유} 거절 사유를 알린다`, async ({ page, request }) => {
