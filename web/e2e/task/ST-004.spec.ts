...
// ST-004 완료되지 않은 작업 보기

test.describe('ST-004 완료되지 않은 작업 보기', () => {
  test('ST-004 AC1: 목록을 열면 완료되지 않은 작업만 보여 준다', async ({ page }) => {
  ...
  test('ST-004 AC2: 해당하는 작업이 없으면 고를 일이 없음을 알린다', async ({ page }) => {
    await 빈_계정으로_바꾼다(page);
  ...
  test('ST-004 AC4: 해당하는 작업이 없으면 다음 행동을 안내한다', async ({ page }) => {
    await 빈_계정으로_바꾼다(page);
  ...
  test('ST-004 AC3: 모르는 스마트 목록을 요청하면 완료되지 않은 작업 전체를 보여 준다', async ({ page }) => {
    await 빈_계정으로_바꾼다(page);
