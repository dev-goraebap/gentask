...
// TG-009.01 이메일을 확인하고 가입하기

test.use(로그인_전);

test.describe('TG-009.01 이메일을 확인하고 가입하기', () => {
  test('TG-009.01 #2: 받은 코드를 제출하면 계정을 만들고 작업 목록을 보여 준다', async ({
    page,
  }) => {
  ...
  test('TG-009.01 #4: 비밀번호가 규칙에 맞지 않으면 충족하지 못한 조건을 알린다', async ({
    page,
  }) => {
  ...
  test('TG-009.01 #5: 제출한 코드가 맞지 않으면 코드가 맞지 않음을 알린다', async ({ page }) => {
