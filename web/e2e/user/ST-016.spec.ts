...
test.describe('ST-016 프로필 이미지 올리기', () => {
  test('ST-016 AC1: 이미지를 올리면 아바타 자리에 그 이미지가 온다', async ({ page }) => {
  ...
  test('ST-016 AC3: 이미지를 지우면 아바타가 이니셜로 돌아온다', async ({ page }) => {
    await 빈_계정으로_바꾼다(page);
    await page.goto('/account');
    await 이미지를_올린다(page);
    await expect(아바타(page).locator('img')).toBeVisible();
  ...
  for (const { 사유, 파일, 문구 } of 거절될_파일) {
    test(`ST-016 AC2: ${사유} 아바타 자리가 그대로 남는다`, async ({ page }) => {
      await 빈_계정으로_바꾼다(page);
      await page.goto('/account');
    ...
    test(`ST-016 AC4: ${사유} 거절 사유를 알린다`, async ({ page }) => {
      await 빈_계정으로_바꾼다(page);
      await page.goto('/account');
