...
test.describe('TG-009.03 비밀번호 바꾸기', () => {
  test('TG-009.03 #1: 현재 비밀번호와 새 비밀번호를 제출하면 비밀번호를 바꾼다', async ({
    page,
  }) => {
  ...
  test('TG-009.03 #2: 바꾸어도 지금 쓰는 자리의 로그인은 유지된다', async ({ page }) => {
  ...
  test('TG-009.03 #4: 현재 비밀번호가 맞지 않으면 아무것도 바꾸지 않는다', async ({ page }) => {
    const email = await 계정_자리로_간다(page);
  ...
  test('TG-009.03 #5: 새 비밀번호가 규칙에 맞지 않으면 충족하지 못한 조건을 알린다', async ({
    page,
  }) => {
    await 계정_자리로_간다(page);
  ...
  test('TG-009.03 #6: 새 비밀번호가 지금 것과 같으면 달라야 함을 알린다', async ({ page }) => {
    await 계정_자리로_간다(page);
