...
//
// TG-008.01 의 AC4 와 TG-008.02 의 다섯은 [서버]다. 자기 줄에는 권한을 바꾸는 단추가 없고, 알림 문제는
// 실패한 발송이 있어야 목록이 서는데 그것을 브라우저로 만들 수 없다 — 스케줄러가 1분 주기라 기다릴 수
// 없다. 아래 마지막 시험은 그 자리가 화면에 서는 것만 지키며 인수 조건을 갖지 않는다.

test.describe.configure({ mode: 'serial' });

test.describe('TG-008 플랫폼 운영', () => {
  test('TG-008.01 #5: 관리자가 아니면 관리 화면에 들어가지 못한다', async ({ page }) => {
    // 워커의 계정은 일반 사용자다. 관리자로 바꾸지 않은 채 곧바로 연다
    await page.goto('/admin');

    await expect(page).toHaveURL(/\/tasks\//);
    await expect(page.getByRole('heading', { name: '관리' })).toHaveCount(0);
  });

  test('TG-008.01 #1: 관리 화면은 가입한 사용자를 최근 가입 순으로 보여 준다', async ({ page }) => {
  ...
  test('TG-008.01 #2: 검색어를 넣으면 그 사용자만 남는다', async ({ page }) => {
  ...
  test('TG-008.01 #3: 사용자를 관리자로 올리면 그 자리에 관리자로 표시된다', async ({ page }) => {
    const 표식 = randomUUID().slice(0, 8);
    const email = await 일반_사용자를_만든다(page, 표식);
    await 관리자로_들어간다(page);
    ...
    const 그_줄 = page.getByRole('row').filter({ hasText: email });
    await expect(그_줄.getByText('사용자', { exact: true })).toBeVisible();

    await 그_줄.getByRole('button', { name: '관리자로' }).click();

    await expect(그_줄.getByText('관리자', { exact: true })).toBeVisible();
    await expect(그_줄.getByRole('button', { name: '사용자로' })).toBeVisible();
  });
  ...
  test('알림 문제 자리가 관리 화면에 선다', async ({ page }) => {
    ...
    await page.goto('/admin');
    await page.getByRole('tab', { name: '알림 문제' }).click();
