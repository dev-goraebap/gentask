...
test.describe('미리 알림 있는 작업 추가', () => {
  ...
  test('알릴 시각을 고르고 확정하면 미리 알림이 붙은 작업을 목록에 넣는다', async ({
    page,
  }) => {
  ...
  test('목록에서 그 작업의 미리 알림을 보여 준다', async ({ page }) => {
    await 미리_알림을_고른다(page);
  ...
  test('기한을 정하지 않았어도 미리 알림만 붙인 작업을 받는다', async ({ page }) => {
    await 미리_알림을_고른다(page);
