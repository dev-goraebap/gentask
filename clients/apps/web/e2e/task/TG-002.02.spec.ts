...
// TG-002.02 기한 있는 작업 추가

test.describe('TG-002.02 기한 있는 작업 추가', () => {
  test.beforeEach(async ({ page }) => {
    await 빈_계정으로_바꾼다(page);
    await page.goto('/tasks/all');
  });

  test('TG-002.02 #1: 기한을 고르고 확정하면 기한이 붙은 작업을 목록에 넣는다', async ({ page }) => {
  ...
  test('TG-002.02 #2: 목록에서 그 작업의 기한을 보여 준다', async ({ page }) => {
    await page.getByRole('button', { name: '기한 설정' }).click();
    await page.getByRole('button', { name: '오늘', exact: false }).first().click();
