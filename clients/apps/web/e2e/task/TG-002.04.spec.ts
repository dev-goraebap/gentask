...
// TG-002.04 스마트 목록으로 작업 보기
//
// 완료된 작업은 별도의 스마트 목록이 아니라 각 목록 하단의 접힌 목록이다.
// 그래서 AC1 은 그 목록이 무엇을 담는가를 묻는다. 완료 자체는 TG-002.10 이 갖는다.
...
test.describe('TG-002.04 스마트 목록으로 작업 보기', () => {
  test('TG-002.04 #1: 완료 목록을 펼치면 보고 있는 스마트 목록의 완료된 작업만 나온다', async ({
    page,
  }) => {
  ...
  test('TG-002.04 #2: 나의 하루 목록은 나의 하루에 추가된 완료되지 않은 작업만 보여 준다', async ({
    page,
  }) => {
    await 빈_계정으로_바꾼다(page);
  ...
  test('TG-002.04 #3: 중요 목록은 중요하다고 표시한 완료되지 않은 작업만 보여 준다', async ({
    page,
  }) => {
    await 빈_계정으로_바꾼다(page);
  ...
  test('TG-002.04 #4: 계획된 일정 목록은 기한이 있는 완료되지 않은 작업만 보여 준다', async ({
    page,
  }) => {
    await 빈_계정으로_바꾼다(page);
