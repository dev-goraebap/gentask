...
test.describe('이 기기로 알림 받기', () => {
  test('등록이 끝나면 이 기기가 알림을 받는 상태임을 보여 준다', async ({ page }) => {
  ...
  test('권한을 거절하면 브라우저 설정에서 다시 허용할 수 있음을 알린다', async ({
    page,
  }) => {
  ...
  test('서버가 이 자리를 모르면 브라우저에 구독이 남아 있어도 받지 않는 상태로 보여 준다', async ({
    page,
  }) => {
  ...
  test('웹 푸시를 지원하지 않으면 이 기기에서 받을 수 없음을 알린다', async ({
    page,
  }) => {
