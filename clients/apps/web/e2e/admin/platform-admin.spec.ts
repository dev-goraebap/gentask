...
//
// 알림 문제 목록은 브라우저로 만들지 못한다. 실패한 발송이 있어야 목록이 서는데 —
// 스케줄러가 1분 주기라 기다릴 수 없다. 아래 마지막 시험은 그 자리가 화면에 서는 것만 지키며 인수
// 조건을 갖지 않는다.

test.describe.configure({ mode: 'serial' });

test.describe('플랫폼 운영', () => {
  test('관리자가 아니면 관리 화면에 들어가지 못한다', async ({ page }) => {
  ...
  test('관리 화면은 가입한 사용자를 최근 가입 순으로 보여 준다', async ({ page }) => {
   ...
   * 들어갈 수 있는지는 위의 권한 시험이 지킨다. 메뉴 하단의 전환 단추를 누르던 시험이
