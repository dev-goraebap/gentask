...
test.describe('TG-056 작업 아이템을 지운다', () => {
  test('TG-056 #1: 지우겠다고 하면 무엇을 지우는지 보이고 되묻는다', async ({ page, request }) => {
  ...
  test('TG-056 #2: 지우기를 확인하면 지우고 목록으로 되돌린다', async ({ page, request }) => {
  ...
  test('TG-056 #3: 되묻는 자리를 그만두면 아무것도 지우지 않는다', async ({ page, request }) => {
  ...
  test('TG-056 #4: 자식이 딸려 있으면 지우지 않고 최상위로 올린다', async ({ page, request }) => {
