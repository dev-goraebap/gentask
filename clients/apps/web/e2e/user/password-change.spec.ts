...
test.describe('비밀번호 바꾸기', () => {
  test('현재 비밀번호와 새 비밀번호를 제출하면 비밀번호를 바꾼다', async ({
    page,
  }) => {
  ...
  test('현재 비밀번호가 맞지 않으면 아무것도 바꾸지 않는다', async ({ page }) => {
    const email = await 계정_자리로_간다(page);
  ...
  test('새 비밀번호가 지금 것과 같으면 달라야 함을 알린다', async ({ page }) => {
    await 계정_자리로_간다(page);
