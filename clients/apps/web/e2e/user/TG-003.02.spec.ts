...
test.describe('TG-003.02 프로필 보기와 계정 자리', () => {
  test('TG-003.02 #1: 계정 화면은 프로필 자리에 아바타를 둔다', async ({ page }) => {
    await page.goto('/account');

    await expect(page.getByRole('main').locator('app-user-avatar')).toBeVisible();
  });

  test('TG-003.02 #2: 올린 이미지가 없으면 아바타 자리에 별명의 첫 글자가 온다', async ({ page }) => {
  ...
  test('TG-003.02 #6: 계정 화면은 이메일과 별명과 가입일을 함께 보여 준다', async ({ page, 계정 }) => {
  ...
  test('TG-003.02 #3: 별명을 고치면 프로필의 이름 표시가 바뀐다', async ({ page }) => {
    await 빈_계정으로_바꾼다(page);
    await page.goto('/account');
  ...
  test('TG-003.02 #7: 별명을 고치면 사이드바의 이름 표시도 바뀐다', async ({ page }) => {
    await 빈_계정으로_바꾼다(page);
    await page.goto('/account');
  ...
  test('TG-003.02 #8: 사이드바의 내 프로필을 고르면 계정 화면으로 간다', async ({ page, 계정 }) => {
