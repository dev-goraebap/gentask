...
test.describe('TG-009.02 비밀번호 재설정하기', () => {
  test('TG-009.02 #2: 코드와 새 비밀번호를 제출하면 새 비밀번호로 로그인하게 한다', async ({
    page,
  }) => {
  ...
  test('TG-009.02 #5: 새 비밀번호가 규칙에 맞지 않으면 충족하지 못한 조건을 알린다', async ({
    page,
  }) => {
    const email = await 재설정을_시작한다(page);
  ...
  test('TG-009.02 #6: 제출한 코드가 맞지 않으면 코드가 맞지 않음을 알린다', async ({ page }) => {
    const email = await 재설정을_시작한다(page);
