 ...
 * 두 개정의 본문을 줄 단위로 견준다.
 *
 * 서버는 차이를 담지 않는다. 담아 두면 본문과 차이 두 자리가 어긋날 수 있고, 어긋났을 때 어느
 * 쪽이 참인지 판정할 근거가 없기 때문이다(DOC-004). 그래서 볼 때 이 자리가 센다.
 *
 * 새 꾸러미를 들이지 않고 직접 적는다. 줄 단위 최장 공통 부분수열은 짧고, 이 화면 하나를 위해
...
export interface DiffResult {
   ...
   *
   * 참이면 지운 줄 전부와 더한 줄 전부만 담긴다. 표가 너무 커져 세는 값이 브라우저를 멈추게 할
 ...
 * 표의 칸 수 상한.
 *
 * 앞뒤로 같은 줄을 걷어 낸 뒤의 넓이를 잰다. 100_000 자 상한인 본문이 전부 달라도 줄이 5 만을
 ...
 *
 * 뒤에서부터 표를 채우고 앞에서부터 거슬러 읽는다. 한 자리에서 갈릴 때 지운 줄을 먼저 내는 것은
