 ...
 * 작업 아이템.
 *
 * 화면이 다니는 식별자는 사람이 부르는 이름(`GT-30`)이고 API 가 받는 것은 프로젝트 안의 번호다.
 * 상위 작업 항목 키와 일련번호를 조합하여 연결한다.
 */
@Injectable()
export class IssueService {
  ...
  /**
   * 생성된 작업 항목 표시 키(예: GT-43)를 반환한다.
   *
   * 작업 표시 키는 서버에서 생성된 키를 사용한다.
   ...
   * 지운다.
   *
   * 되묻는 것은 화면이 한다. 자식은 함께 지워지지 않고 최상위로 올라오므로(ITM-005 A2) 목록을
   ...
   * 상세 하나를 싣는다.
   *
   * 목록의 줄은 본문과 인수 조건을 갖지 않으므로 상세는 따로 묻는다. 주입 자리에서 불러야 하며,
