 ...
 *
 * 웹폰트가 아니라 SVG 로 둔다. 폰트는 전체 세트가 660KB 이고 우리가 쓰는 것은 서른몇 개뿐이며,
 * 글리프가 코드포인트라 폰트가 도착하기 전 한자로 보이는 자리를 따로 가려야 한다. SVG 는 쓰는 것만
 * 번들에 들어가고 그 문제가 없다.
 *
 * 이름을 우리 어휘로 다시 짓지 않고 `hgi` 접두어와 원래 이름을 쓴다. 그림을 바꾸려 할 때 어느
 * 아이콘이었는지 찾을 수 있어야 한다.
 *
 * 배럴이 아니라 아이콘 파일을 하나씩 가리킨다. 패키지의 진입점은 6천여 개를 다시 내보내며, 그것을
 ...
 * 화면이 부르는 이름과 그림의 대응.
 *
 * `provideIcons` 가 받는 것은 SVG 문자열이므로 여기서 옮겨 담는다.
 */
