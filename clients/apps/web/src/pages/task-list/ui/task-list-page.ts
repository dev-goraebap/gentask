...
export class TaskListPage {
   ...
   *
   * 늘 열어 두면 바닥의 띠와 함께 화면의 아래를 두 겹으로 먹는다. 적는 일은 잠깐이고 보는 일은
   ...
   * 방금 완료를 눌러 목록에서 빠질 행.
   *
   * 사라지는 것을 그리는 대상을 이 하나로 좁힌다. 모든 행에 걸면 뷰를 옮길 때 옛 목록 전체가
  ...
  constructor() {
     ...
     * 두 번 누르게 된다.
     *
     * 효과가 그 자리인 이유는 감춰진 요소에 포커스를 줄 수 없기 때문이다. 여는 순간에 바로 부르면
   ...
   *
   * 초점을 잃는 그 순간에 접으면 제 안의 단추를 삼킨다. 기한과 미리 알림은 이 폼 안에
   * 있어서, 제목을 적기 전에 그것을 누르면 초점이 옮겨 가는 사이에 폼이 사라져 누르려던 것이
   * 없어진다. 초점이 어디에 앉았는지 본 뒤에 판단한다.
   *
   * 덮개도 함께 본다. 데이트피커는 오버레이로 뜨므로 그 안에 앉은 초점은 폼 밖에 있다.
   */
