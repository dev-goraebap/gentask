...
export function TaskArtifacts({task,writable}:{task:Task;writable:boolean}){
 ...
 {linked.isPending||linked.error?<RequestState error={linked.error} retry={()=>void linked.refetch()}/>:linked.data.map(artifact=><HStack key={artifact.id} gap={1} justify="between" align="center"><Button label={artifact.title} href={(task.projectId?'/projects/'+task.projectId:'')+'/artifacts/'+artifact.id} variant="secondary" style={{minWidth:0,overflow:'hidden',textOverflow:'ellipsis'}}/>{writable?<Button label="연결 해제" variant="secondary" size="sm" isDisabled={action.isPending} onClick={()=>action.mutate(()=>linkTaskArtifact(task.id,artifact.id,false))}/>:null}</HStack>)}
