...
export function TaskArtifacts({task,writable}:{task:Task;writable:boolean}){
 ...
 {linked.isPending||linked.error?<RequestState error={linked.error} retry={()=>void linked.refetch()}/>:linked.data.map(artifact=><HStack key={artifact.id} gap={1} justify="between" align="center"><Button label={artifact.title} href={(task.projectId?'/projects/'+task.projectId:'')+'/artifacts/'+artifact.id} variant="ghost" style={{minWidth:0,overflow:'hidden',textOverflow:'ellipsis'}}/>{writable?<Button label="연결 해제" variant="ghost" size="sm" isDisabled={action.isPending} onClick={()=>action.mutate(()=>linkTaskArtifact(task.id,artifact.id,false))}/>:null}</HStack>)}
 {linked.data?.length===0?<Text type="supporting">연결된 아티팩트가 없습니다.</Text>:null}
 {writable?<Selector label="아티팩트 연결" isLabelHidden value="" placeholder="연결할 아티팩트 선택" hasSearch options={(available.data??[]).filter(a=>!linked.data?.some(l=>l.id===a.id)).map(a=>({value:a.id,label:a.title}))} isDisabled={action.isPending||!linked.data||!!available.error} isLoading={available.isPending} onChange={id=>action.mutate(()=>linkTaskArtifact(task.id,id,true))}/>:null}
