...
import { Button, Text, TextInput, TextArea, DateInput, Selector, VStack } from '@astryxdesign/core';
...
export function TaskForm({ task, onDeleted }: { task: Task; onDeleted: () => void }) {
  ...
  return <VStack padding={3} gap={5} style={{ overflowY: 'auto', flex: 1, minHeight: 0 }}>
    <VStack gap={3}>
      ...
      <TextInput label="제목" isLabelHidden placeholder="작업 제목" value={autosave.values.title}
        onChange={(value, event) => autosave.changeText('title', value, (event?.nativeEvent as InputEvent | undefined)?.isComposing)}
        onCompositionEnd={event => { if (event.target instanceof HTMLInputElement) autosave.changeText('title', event.target.value); }} onBlur={() => autosave.flush('title')}
        status={autosave.status('title')} isReadOnly={!writable} isDisabled={action.isPending} />
      <TextArea label="설명" isLabelHidden placeholder="설명을 입력하세요" value={autosave.values.note}
        ...
        status={autosave.status('note')} rows={5} isReadOnly={!writable} isDisabled={action.isPending} />
