import { Button, Heading, HStack, Popover, Text, TextInput, VStack } from '@astryxdesign/core';
...
export function TaskNotes({task,writable}:{task:Task;writable:boolean}) {
  ...
  const add=()=>{if(/^[A-Za-z0-9_-]{12}$/.test(id.trim())&&!action.isPending) action.mutate(async()=>{await linkTaskNote(task.id,id.trim(),true);setId('');setOpen(false);});};
  return <VStack gap={2}>
    ...
    {writable ? <Popover label="메모 연결" isOpen={open} onOpenChange={setOpen} width="min(20rem, calc(100vw - 2rem))" content={<VStack gap={3} padding={3}>
      ...
      {action.error ? <Text role="alert" type="supporting">{action.error.message}</Text> : null}
    </VStack>}><Button label="메모 연결…" variant="ghost" size="sm" /></Popover> : null}
