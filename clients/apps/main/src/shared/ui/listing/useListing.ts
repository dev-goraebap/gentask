...
export interface ListingState { page: number; size: number; query: string; filter: string; sort: string; scroll: number; mobileScroll: number; loaded: number; scope: string }
...
export function useListing(key: string, scope = '') {
  const initial = (): ListingState => ({ page: 1, size: 25, query: '', filter: 'all', sort: 'title', scroll: 0, mobileScroll: 0, loaded: 25, scope });
  ...
  const change = (patch: Partial<ListingState>) => {
    const resets = ['query', 'filter', 'sort', 'size'].some(field => field in patch);
  ...
  }, [key, scope, state.page, state.query, state.filter, state.sort, state.size, state.loaded, mobile]);
