...
export function CreateButton({ label, onClick }: { label: string; onClick: () => void }) {
  ...
  return <Button label={label} icon={<HgiPlus />} variant="primary" size={mobile ? 'lg' : 'sm'}
    ...
    style={mobile ? { position: 'fixed', insetInlineEnd: 'var(--spacing-4)', width: 'var(--spacing-12)', height: 'var(--spacing-12)', bottom: 'calc(var(--spacing-4) + env(safe-area-inset-bottom))', zIndex: 5 } : undefined} />;
}
