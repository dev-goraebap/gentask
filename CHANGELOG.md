...
- OTP 발송 rate limit — IP별 촘촘히·이메일별 넉넉히(비대칭, 결정-0015 §결정 3). 인메모리 고정 윈도우 구현이며 포트 뒤에 있어 다중 인스턴스 배포 시 공유 저장소 구현으로 교체한다 (#28)
...
- 인증 테스트 55건 — 도메인 불변식(OTP 만료·5회 폐기·1회용, 슬라이딩 만료, credential 불변식), 서비스 단위(가짜 저장소, 실패 사유 비구분·이메일 비예약), 통합(가입→로그인→로그아웃 전체 흐름, Bearer 경로, Origin 검증, 저장소 왕복). OTP 코드는 항상 발송된 메일에서 관찰한다 (#28)
