...
- Lombok 사용 범위를 층별로 명문화 (결정-0023) — Spring 빈은 `@RequiredArgsConstructor`, 도메인 객체는 손으로 쓴 생성자(불변식을 지키는 자리이므로), DTO는 record, 로거는 `@Slf4j`. `@Getter`·`@Data`·`@Builder`는 쓰지 않는다. 그동안 방침이 없어 `@Slf4j`만 8곳에 쓰이고 생성자 주입 14곳은 전부 손으로 쓰인 상태였다. 기존 14곳의 전환은 #35
