...
- 제공자 인증과 이메일 증명 사이의 중간 상태를 **서명된 짧은 표**로 잇는다 — 이 시점에는 이메일을 모르니 대기 레코드를 만들 수 없고, 이메일 없이 만들 수 있게 스키마를 열면 "가입 대기는 이메일을 향한다"는 성질이 흐려진다. 표는 자격증명이 아니라 배턴이다: 훔쳐도 OTP를 통과하지 못하면 계정이 생기지 않으며 10분이면 만료된다 (#34)
...
- 제공자 토큰을 **저장하지 않는다** — 원래 인수조건은 `account`에 보관하는 것이었으나, 이메일 scope조차 요구하지 않아 제공자 API를 호출할 계획이 없다. 쓰지 않을 자격증명을 들고 있으면 우리 DB 유출이 남의 서비스 접근으로 번진다. `accounts`의 토큰 칸은 스키마에 그대로 두어 필요해질 때 채운다 (#34)
