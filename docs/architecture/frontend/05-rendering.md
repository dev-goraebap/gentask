...
이 문서는 [06. 레이아웃](06-layout.md)과 [07. 적응형 UI](07-adaptive-ui.md)보다 앞섭니다. 경로의 렌더링 모드가 정해져야 그 경로에서 적응형 컴포넌트를 쓸 수 있는지 판정됩니다.
...
정적 생성 대상은 **명시적으로 열거한 경로만** 받습니다. `**` 를 `Client` 로 설정하여 신규 라우트가 기본적으로 클라이언트 렌더링으로 안전하게 폴백되도록 합니다.
...
>
> `angular.json` 의 `security.allowedHosts` 가 빈 배열이면 모든 요청이 400 으로 거부됩니다. 정적 생성 단계에서는 감지되지 않으며, 실제 SSR 서버 기동 시 요청이 모두 차단됩니다.
...
**`afterNextRender` 안에서 화면 구성을 바꾸지 않습니다.** 정적 생성 시점에는 실행되지 않으므로 그 결과가 생성된 HTML 에 누락되고 하이드레이션 이후에야 나타납니다. 고정된 자리에 요소가 뒤늦게 삽입되어 레이아웃 시프트가 발생하며, 개발 서버에서는 로딩 속도가 빨라 결함 감지가 지연될 수 있습니다.
...
| 정적 생성 경로에서 적응형 컴포넌트 사용 | 서버에는 뷰포트가 없습니다 |
...
>
> 서버에는 뷰포트가 없어 `injectViewportClass()` 가 기본값 `wide` 를 반환합니다. 빌드 시 생성된 HTML 과 클라이언트 판정이 어긋나면 화면이 한 번 깜빡이며 다시 그려집니다. 공개 경로에는 CSS 미디어 쿼리로 해결되는 반응형만 사용합니다. **이 위반은 예외로 드러나지 않으므로** 코드 리뷰에서 확인합니다. 상세는 [07. 적응형 UI](07-adaptive-ui.md)에 있습니다.
...
[프로세스](../../process.md)의 2단계에서는 데이터가 Mock 구현 뒤에 있습니다. Mock 은 브라우저와 Node 양쪽에서 동작하므로 렌더링 모드 선택에 제약을 주지 않습니다.
...
| 용어 | 정의 |
| :--- | :--- |
| **하이드레이션 (Hydration)** | 서버가 생성한 HTML 에 클라이언트가 이벤트와 상태를 결합해 상호작용 가능한 상태로 만드는 과정 |
| **증분 하이드레이션** | 화면 전체가 아니라 필요한 블록만 선택적으로 하이드레이션하는 방식. Angular 22 에서 기본 활성화 |
| **전송 상태 (Transfer State)** | 서버 렌더링 시 조회한 데이터를 HTML 에 실어 클라이언트로 전달해 중복 요청을 막는 기법 |
