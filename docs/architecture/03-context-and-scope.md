...
flowchart LR
    ...
    agent -- "HTTPS · gentask.xyz" --> gentask
    gentask -- "JDBC" --> db
...
- **`gentask.xyz/api/`**: 작업 JSON 데이터 전송. Bearer 토큰을 통해 인증한다.
...
- **단일 출처 기반 인증 구조**: 웹 화면과 REST API가 동일한 출처(`gentask.xyz`)를 공유함으로써 브라우저의 세션 쿠키를 활용한다. 브라우저 밖의 클라이언트(로컬 에이전트 등)는 같은 출처의 `/api/` 경로에 Bearer 토큰으로 접근한다.
