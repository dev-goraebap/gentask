...
아키텍처 및 코드 작성 규칙의 원본은 [5. 빌딩 블록 뷰](../05-building-block-view.md), [백엔드 02. 코드 작성 규약](./02-code-style.md)에 정의되어 있으며, 본 문서는 각 규칙이 빌드 및 린트 도구에서 강제되는 지점과 환경 요건을 명시합니다.
...
| **ArchUnit** | 패키지 의존성 및 아키텍처 계층 구조 규칙 검증 | 단위 테스트 단계에서 실행 (규칙 원본: [5.3 레벨 2 — server](../05-building-block-view.md)) |
