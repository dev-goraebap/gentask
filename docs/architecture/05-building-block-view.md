...
- **`web`** (`clients/apps/web/src`): 사용자 인터페이스 및 상호작용 처리. 빌드 시점에 정적 산출물을 생성하며, 런타임 렌더링과 상태 관리는 브라우저에서 수행합니다.
...
계층 구성은 고정이며, 계층 내부의 슬라이스 목록은 기능 요구사항에 따라 변동되므로 본 명세에는 개별 슬라이스를 전수 나열하지 않습니다. 현재 구성된 슬라이스 목록은 `clients/apps/web/src/` 디렉터리 구조를 참조합니다.
