...
코드 배치 규격은 코드 위치의 일의성(Uniqueness)을 보장하기 위해 정의합니다. 동일한 성격의 코드가 서로 다른 위치에 분산 배치되는 것을 방지하며, 본 절의 판정 기준을 최우선으로 적용합니다. 자동화 검증이 가능한 규칙은 빌드 및 린트 단계에서 강제하며, 세부 강제 수단과 설정은 [8. 횡단 관심사](./08-crosscutting-concepts.md)에서 다룹니다.
...
**`widgets` 계층의 생성을 금지합니다.** 화면 전용 조합 컴포넌트의 계층 분류 판단 비용을 최소화하고 구조를 단순하게 유지하기 위해 위젯 계층을 사용하지 않습니다. 재사용되지 않는 화면 단위 컴포넌트 조합은 `pages/<화면>/` 내부에서 직접 구성합니다.

`processes` 계층은 FSD v2.1 명세에서 폐기(deprecated)되었으므로 사용을 금지합니다.
...
새로운 코드의 배치 위치는 아래 순서에 따라 순차적으로 판정하며, 최초로 일치하는 조건에 배치합니다.

```text
1. 애플리케이션 전역 설정인가?                  → app/
2. 비즈니스 로직이 없는 범용 인프라인가?          → shared/<세그먼트>/
3. 특정 단일 화면에서만 사용하는가?               → pages/<화면>/
4. 2곳 이상의 화면에서 이미 사용하는 사용자 동작인가? → features/<동작명>/
5. 2곳 이상의 화면에서 이미 사용하는 도메인 모델인가? → entities/<도메인명>/
6. 판정이 불확실한 경우                         → pages/<화면>/
```

**6번 항목이 기본 판정 규칙입니다.** 배치 위치가 불명확한 경우 항상 `pages/<화면>/`에 배치합니다. 화면 간 코드 중복은 허용되며, 단순 중복이나 향후 재사용 가능성(가설적 재사용)을 근거로 상위/공유 계층으로 선제 추출하는 것을 금지합니다.

4번 및 5번의 조건은 **현재 2곳 이상의 서로 다른 화면에서 실제로 참조·호출되고 있는 경우**를 의미하며, 추후 재사용 예상은 승격 조건에 포함되지 않습니다.

2번의 인프라 항목은 UI 킷(`shared/ui/`), 범용 유틸리티(`shared/lib/`), HTTP 클라이언트 및 OpenAPI 기반 생성 타입(`shared/api/`) 등 도메인 지식이 배제된 범용 기술 요소를 의미합니다. 특정 도메인의 리소스를 조회·조작하는 로직은 해당 도메인 슬라이스(`entities/<도메인명>/api/`)에 배치합니다.

`shared` 계층에 배치되는 범용 인프라 코드는 사용처 개수(2곳 이상) 조건을 적용받지 않으며 즉시 배치가 허용됩니다.
...
계층 간 임포트는 TypeScript 경로 별칭(`@/*`)을 사용합니다. 상대 경로는 동일 슬라이스(또는 세그먼트) 내부 참조에 한해 허용합니다. `tsconfig.json`의 `baseUrl` 설정은 TypeScript 컴파일 오류(`TS5101`)를 유발하므로 금지하며, `paths` 매핑만을 사용합니다.

`providedIn: 'root'` 선언은 `app` 및 `shared` 계층의 전역 싱글턴 서비스에만 허용합니다. `pages` 슬라이스에 속한 화면 전용 서비스는 라우트 정의의 `providers` 배열에 등록하여 화면 진입·이탈 수명주기와 서비스 수명을 일치시킵니다.

### 5.2.5 승격과 강등

동일한 코드가 **2개 이상의 서로 다른 화면(`pages`)에서 실제로 참조되는 시점**에 상위 계층으로 승격합니다. 사용자 인터랙션 및 UI 조합은 `features/<동작명>/`, 도메인 모델 및 규칙은 `entities/<도메인명>/`으로 승격합니다. 승격 시에는 코드를 대상 슬라이스의 세그먼트로 이동하고, `index.ts` 공개 API를 통해 필요한 인터페이스만 내보낸 후 호출부의 임포트 경로를 갱신합니다.

슬라이스 강등은 Steiger 린터의 `insignificant-slice` 규칙 검출 시 수행합니다. 단일 화면에서만 참조되는 슬라이스는 해당 `pages/<화면>/` 내부로 강등하여 관리합니다. 단일 사용처 슬라이스를 원래 화면으로 환원하는 것은 정상적인 리팩터링 절차로 규정합니다.
...
```text
module/<모듈>/
├── contract/            타 모듈에 공개하는 인터페이스 및 DTO
...
| 패키지 | 담는 것 |
| :--- | :--- |
| **`contract`** | 타 모듈에 공개하는 서비스 인터페이스 및 반환/요청 DTO |
...
**`contract` 패키지에 위치한 요소만 타 모듈에 공개되며, 나머지 패키지는 모듈 내부 전용입니다.** 타 모듈과의 협업 인터페이스가 필요한 경우에 한해 `contract` 패키지를 생성하고 공개 인터페이스를 배치합니다. 타 모듈에 노출할 인터페이스가 없는 독립 모듈은 `contract` 패키지를 생성하지 않습니다.

### 5.3.3 하위 패키지 승격

신규 모듈 생성 시 각 계층 하위에 파일을 직접 평면 배치하며, 하위 패키지 분할(승격)은 **각 계층의 분할 축에 따라 독립적으로 판정**합니다.
...
`domain`과 `application` 계층은 분할 축이 서로 다르므로 승격 임계치를 독립적으로 계산합니다. 애그리거트 개수는 `domain` 계층의 분할 기준이며, `application` 계층의 분할 기준이 아닙니다. 하나의 유스케이스가 복수 애그리거트의 저장소를 참조할 수 있으므로, 애그리거트와 서비스의 1:1 대응을 전제하지 않습니다. 애그리거트가 4개이더라도 유스케이스가 2개인 경우 `application` 계층은 평면 구조를 유지합니다.

2개 이상의 피쳐에서 공통으로 참조하는 요소는 **해당 계층 루트에 직접 배치**합니다.

승격이 적용된 `application` 계층 구조의 예시는 다음과 같습니다.

```text
module/<모듈>/application/
├── <피쳐 A>/                  컨트롤러 · 서비스 · 요청 DTO · 응답 DTO · 조회 포트
├── <피쳐 B>/                  컨트롤러 · 서비스 · 요청 DTO · 응답 DTO · 조회 포트
└── <모듈>ErrorCode.java       복수 피쳐가 공통으로 참조하므로 계층 루트에 배치
```

`application/shared/` 또는 `application/common/`과 같은 모호한 공용 하위 패키지 생성을 금지합니다. 공통 요소는 계층 루트에 직접 노출하여 가시성을 확보하며, 루트에 공통 파일이 과도하게 누적될 경우 하위 패키지 신설 대신 피쳐 경계의 재조정을 우선 검토합니다.

`infrastructure` 계층은 피쳐 단위로 분할하지 않고, 대응하는 포트 인터페이스 명칭을 따르는 구현체를 평면 배치합니다. 단, 포트 구현체가 아닌 기술 인프라 확장 요소(예: 웹 인터셉터 및 관련 등록 설정)는 목적을 명시한 하위 패키지(예: `infrastructure/web/`)로 그룹화합니다.
...
| **서비스** | 타 모듈 `contract` | 허용 | 모듈 간 공식 공개 계약 참조 |
| **서비스** | 타 모듈의 `contract` 외 패키지 | **금지** | 모듈 내부 캡슐화 보장 및 결합도 격리 |
...
| **인프라** | 타 모듈 `contract` | 허용 | 타 모듈 연동 어댑터(ACL) 구현 |
