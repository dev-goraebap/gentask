...
**개발용 데이터베이스와 파일 스토리지는 로컬 컨테이너로 구동하지 않는다.** 홈서버의 공용 PostgreSQL 내 `gentask_dev` 데이터베이스와 MinIO 버킷 `gentask-dev`를 바라본다. 상세 배경은 [결정-0014](../decisions/0014-backend-development-backing-services.md)에 기술되어 있다.

```bash
./gradlew bootRun
```

로컬에서 별도의 백킹 서비스 컨테이너를 구동할 필요 없이 `.env`에 설정된 원격 엔드포인트로 즉시 연결된다.

**파일 스토리지는 인가된 네트워크 환경에서만 접근할 수 있다.** nginx 리버스 프록시가 개발자의 인가된 공인 IP만 접근을 허용하므로, 미인가 네트워크에서는 인증 정보가 올바르더라도 연결이 거부된다. 이때 웹 화면과 데이터 조회는 정상 동작하지만 파일 첨부 기능만 차단되므로, 업로드 실패 시 프록시 허용 IP 목록을 우선 확인한다. 신규 작업 환경의 공인 IP는 서버 nginx 설정에 추가해야 한다.

모든 외부 접속 정보는 `server/.env` 파일에서 단일 관리한다. 애플리케이션은 `spring.config.import`를 통해 환경 변수를 로드하며, 배포 환경에서는 동일한 키를 컨테이너 환경 변수로 주입한다.
...
**데이터베이스는 표준 포트(5432)로 직접 노출되지 않는다.** 홈서버 공유기의 포트 포워딩을 통해 별도 포트로 라우팅되며, 실제 접속 포트는 `.env`에 정의한다.
...
**테스트용 컨테이너는 개발 데이터베이스와 격리하여 실행한다.** 테스트 데이터와 개발 데이터의 간섭을 차단하여 테스트의 독립성과 재현성을 보장한다. 개발 데이터베이스가 원격에 있는 것과 무관하게 테스트는 로컬 컨테이너만 사용하므로, 홈서버에 닿지 않는 환경에서도 `./gradlew build`는 정상 통과한다.

**개발 · 테스트 · 운영의 데이터베이스 메이저 버전을 일치시킨다.** 엔진 버전 차이로 인해 한쪽에서 성공한 마이그레이션이 다른 쪽에서 예기치 않게 실패하는 문제를 방지하기 위함이다. 개발과 운영은 홈서버의 동일 인스턴스를 공유하므로 자동으로 일치하며, 테스트 컨테이너의 이미지 태그만 해당 버전에 맞춘다.
