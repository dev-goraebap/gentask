...
| `access_token` | `varchar(2048)` | null 허용 | 제공자 API 호출용. 클라이언트로 나가지 않는다 |
| `refresh_token` | `varchar(2048)` | null 허용 | 같음 |
| `token_expires_at` | `timestamptz` | null 허용 | |
| `created_at` | `timestamptz` | not null | |
| `updated_at` | `timestamptz` | not null | |
...
| `target_email` | `varchar(320)` | not null | **정규화된** 값 |
