...
- **`current_timestamp` 기본값을 쓰지 않는다.** 시각은 애플리케이션이 주입된 `Clock`으로 채운다
  ([DB · 데이터 접근](../../server/docs/references/DB-데이터-접근.md) §4) — 테스트가 시각을 통제할 수 있어야 한다
...
세션 테이블은 **하나다.** 쿠키 경로와 Bearer 경로가 같은 행을 가리킨다([API 설계](../../server/docs/references/API-설계.md) §5).
...
| `id` | `uuid` | PK | |
| `user_id` | `uuid` | not null, FK → `users` (cascade) | |
| `provider` | `varchar(20)` | not null | `credential` \| `google` \| `kakao` |
...
**`provider_account_id`는 제공자의 subject id다** — 구글 `sub`, 카카오 numeric id.
...
새로 발급한다([API 설계](../../server/docs/references/API-설계.md) §6) — 승격시키면 세션 고정이 된다.
